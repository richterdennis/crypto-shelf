...
export function encrypt(key, str, options = {}) {
	...
	if (authTagLength && aad) {
		cipher.setAAD(Buffer.from(aad, 'utf8'), {
			plaintextLength: Buffer.byteLength(str),
		});
	}
	...
	const encrypted = Buffer.concat([
		...
		cipher.final(),
		authTagLength ? cipher.getAuthTag() : Buffer.alloc(0),
	]);

	return encoding ? encrypted.toString(encoding) : encrypted;
}

export function decrypt(key, str, options = {}) {
	const { algorithm, encoding, aad } = merge(defaults, options);
	const { ivLength, authTagLength, mode } = getInfo(algorithm);

	const encrypted = Buffer.from(str, encoding);
	...
	if (authTagLength && aad) {
		decipher.setAAD(Buffer.from(aad, 'utf8'), {
			plaintextLength: encrypted.length - ivLength - authTagLength,
		});
	}

	if (authTagLength) decipher.setAuthTag(encrypted.subarray(encrypted.length - authTagLength));

	return decipher.update(
		encrypted.subarray(ivLength, encrypted.length - authTagLength),
		'utf8',
	) + decipher.final('utf8');
}
